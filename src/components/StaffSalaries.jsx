...
const StaffSalaries = ({
  ...
  const handleDuringItemChange = (key, field, value) => {
    setDuringDetails(prev => {
      ...
      if (field === 'total') {
        // Update total via slider or total input
        // Keep quantity constant, adjust rate
        updated.rate = value / item.quantity;
      } else if (field === 'rate') {
        // Rate field edited
        updated.rate = value;
      } else if (field === 'quantity') {
        // Quantity field edited
      ...
      <div className="flex-1">
        <div className="flex justify-between items-center mb-1">
          <div className="flex items-center gap-1">
            ...
            >
              {expanded.main ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
            </button>
            <span className="text-sm">Staff Salaries: {formatCurrency(total)}</span>
          </div>
          <button
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-sm">During Semester: {formatCurrency(duringSemester)}</span>
                </div>
              </div>
